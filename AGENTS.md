...
Expected result: 177 passed, 4 skipped, exit code 0.

## Architecture

- `PUnit.pmod/module.pmod` -- re-exports `Assertions.pmod` so `import PUnit` exposes all assert functions
- `PUnit.pmod/Assertions.pmod` -- 28 assertion functions (equality, identity, boolean, null, comparison, containment, type, exceptions, approximate, match, fail, skip, collection) plus `run_process` subprocess helper, each with an optional `_loc` parameter for exact source location
- `PUnit.pmod/macros.h` -- preprocessor macros that inject `__FILE__:__LINE__` into assertions. Includes granular headers (equal.h, boolean.h, comparison.h, null.h, membership.h, exception.h, misc.h, collection.h). Auto-generated by `scripts/generate_macros.pike`.
- Category sub-modules (`Equal.pmod`, `Boolean.pmod`, `Comparison.pmod`, `Null.pmod`, `Membership.pmod`, `Exception.pmod`, `Collection.pmod`, `Misc.pmod`, `Process.pmod`) -- selective `import PUnit.Equal` brings only equality assertions into scope
...
- Selective imports: use `import PUnit.Equal` (or any category) to bring only specific assertions into scope.
