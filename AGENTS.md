...
Expected result: 165 passed, 4 skipped, exit code 0.
...
- Category sub-modules (`Equal.pmod`, `Boolean.pmod`, `Comparison.pmod`, `Null.pmod`, `Membership.pmod`, `Exception.pmod`, `Collection.pmod`, `Misc.pmod`) -- selective `import PUnit.Equal` brings only equality assertions into scope
